...
class RobotBody:
    ...
    def prepare_tts_text(text: str) -> str:
        ...
        if not raw:
            return ""
        max_chars = int(os.getenv("ROBOT_TTS_MAX_CHARS", "240"))
        if len(raw) <= max_chars:
            return raw
        clipped = raw[: max_chars - 1].rstrip()
        last_break = max(
            clipped.rfind("."),
            clipped.rfind("!"),
            clipped.rfind("?"),
    ...
    async def play_tts_async(self, agent, text):
        ...
        try:
            loop = asyncio.get_running_loop()
            speak_text = self.prepare_tts_text(text)
            if not speak_text:
                self.face.set_state(speaking="Skipped", event="No reply to speak")
                await self.channel.emit_event("tts_done", text=text, duration_ms=0.0)
                return
            self.face.set_state(speaking="Synthesizing reply", event="Generating speech")
            tts_started = time.perf_counter()
            audio = await loop.run_in_executor(None, lambda: agent.run(speak_text, model_kind="tts"))
            if audio is None:
                self.face.set_state(speaking="Skipped")
                await self.channel.emit_event("tts_done", text=text, duration_ms=0.0)
                return
