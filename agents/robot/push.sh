...
Notes:
  ...
  --local does the default local path explicitly
  --gcp bootstraps templates for GCP brain deployment
  --idempotent     (default) Publish baseline artifacts + fresh certs (no re-modulation).
  --no-idempotent  Full EC-ANN modulation: regenerate ecm.pt + certs for all models.
                   Use this to fix digest mismatches or after changing hyperparameters.
