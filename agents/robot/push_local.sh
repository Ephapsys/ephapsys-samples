...
USAGE="Usage: $0 [--idempotent|--no-idempotent] [--label \"Robot Agent Template\"]
  --idempotent     (default) Publish baseline artifacts; issue fresh certs without re-modulating.
  --no-idempotent  Full EC-ANN modulation: regenerate ecm.pt + matching certs for all models.
                   Use this to fix digest mismatches or after changing model hyperparameters."
POLL_INTERVAL_SEC=5
POLL_TIMEOUT_SEC=7200
CLI_TOKEN_FILE=".cli_token"
MODULATION_MODE="${ROBOT_MODULATION_MODE:-local}"
ROBOT_GCP_MODULATION_GPU="${ROBOT_GCP_MODULATION_GPU:-t4}"
ROBOT_GCP_MODULATION_ARGS="${ROBOT_GCP_MODULATION_ARGS:-}"
SHARED_GCP_MODULATION_STATE_FILE=""
...
cleanup_shared_gcp_modulation_vm() {
  local state_file="${SHARED_GCP_MODULATION_STATE_FILE:-}"
  [[ -n "$state_file" ]] || return 0
  [[ -f "$state_file" ]] || return 0

  local instance_name="" zone="" project_id=""
  ...
  if [[ -n "$instance_name" && -n "$zone" && -n "$project_id" ]]; then
    echo "[INFO] Cleaning up shared GCP modulation VM: ${instance_name} (${project_id}/${zone})"
    gcloud compute instances delete "$instance_name" --project="$project_id" --zone="$zone" --quiet >/dev/null 2>&1 || true
  fi
  rm -f "$state_file" >/dev/null 2>&1 || true
}

if [[ "$MODULATION_MODE" == "gcp" ]]; then
  SHARED_GCP_MODULATION_STATE_FILE="$(mktemp)"
  export MODULATOR_GCP_SHARED_STATE_FILE="$SHARED_GCP_MODULATION_STATE_FILE"
  trap cleanup_shared_gcp_modulation_vm EXIT
fi
...
run_modulator() {
  ...
  if [[ "$kind" == "vocoder" ]]; then
    publish_only=true
  fi

  local gcp_runner=""
  if [[ "$publish_only" == false && -x "${path}/modulate_gcp.sh" ]]; then
    gcp_runner="./modulate_gcp.sh"
  fi
  local local_runner=""
  if [[ "$publish_only" == false && -x "${path}/modulate.sh" ]]; then
    local_runner="./modulate.sh"
  elif [[ "$publish_only" == false && -x "${path}/modulate_local.sh" ]]; then
    local_runner="./modulate_local.sh"
  fi

  write_modulator_env() {
    cat > "${path}/.env" <<EOF
AOC_BASE_URL=${BASE_URL}
BASE_URL=${BASE_URL}
...
EOF
  ...
  run_gcp_modulator() {
    if [[ -z "$gcp_runner" ]]; then
      return 1
    fi
    write_modulator_env
    echo "[INFO] Running ${kind} modulation on GCP (gpu=${ROBOT_GCP_MODULATION_GPU}, mode=$([[ $IDEMPOTENT -eq 1 ]] && echo idempotent || echo full))"
    local saved_train_mode="${TRAIN_MODE-}"
    pushd "$path" >/dev/null
    ...
    if [[ -n "$ROBOT_GCP_MODULATION_ARGS" ]]; then
      # shellcheck disable=SC2206
      local extra_args=( $ROBOT_GCP_MODULATION_ARGS )
      "$gcp_runner" "$ROBOT_GCP_MODULATION_GPU" "${extra_args[@]}"
    else
      "$gcp_runner" "$ROBOT_GCP_MODULATION_GPU"
    fi
    popd >/dev/null
    if [[ -n "${saved_train_mode:-}" ]]; then
      export TRAIN_MODE="$saved_train_mode"
    else
      unset TRAIN_MODE || true
    fi
    return 0
  }

  if [[ $IDEMPOTENT -eq 1 ]]; then
    # Idempotent mode still needs a real modulator path in GCP mode so the runtime keeps
    # exercising ephaptic coupling instead of collapsing to a pure server-side publish.
    if [[ -z "$mid" ]]; then
      echo "[WARN] No template found for ${kind}; skipping idempotent publish."
      return 1
    fi
    if [[ "$publish_only" == true ]]; then
      echo "[INFO] ${kind} has no standalone modulator; using explicit publish path by design."
    elif [[ "$MODULATION_MODE" == "gcp" ]] && run_gcp_modulator; then
  ...
  # Full modulation mode (--no-idempotent): generate fresh ecm.pt + matching certificates.
  if [[ -z "$mid" ]]; then
    echo "[ERROR] No TEMPLATE found for kind=${kind}"
    return 1
  fi
  if [[ "$publish_only" == true ]]; then
    echo "[INFO] ${kind} has no standalone trainer; using server-side modulation publish path."
    curl -s -X POST "${BASE_URL}/modulation/start" \
      -H "Authorization: Bearer ${API_TOKEN}" \
      -H "Content-Type: application/json" \
      -d "{
  ...
  if [[ "$MODULATION_MODE" == "gcp" ]] && run_gcp_modulator; then
    poll_until_done "$mid" || true
  elif [[ -n "$local_runner" ]]; then
    # Local modulate.sh present: run it (e.g. fine-tuning + EC-ANN coupling).
    write_modulator_env
    pushd "$path" >/dev/null
    "$local_runner"
    popd >/dev/null
    ...
    # No local modulate.sh: fall back to server-side modulation.
    # The platform runs EC-ANN coupling and generates a new certified ecm.pt.
    echo "[INFO] No local modulate.sh for ${kind}; triggering server-side modulation (template=${mid})"
    curl -s -X POST "${BASE_URL}/modulation/start" \
      -H "Authorization: Bearer ${API_TOKEN}" \
      -H "Content-Type: application/json" \
      -d "{
