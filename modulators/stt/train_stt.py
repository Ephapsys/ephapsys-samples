#!/usr/bin/env python3
"""
Trainer script for STT (speech-to-text) with ephaptic coupling integration.

This trainer streams per-step WER metrics back to the AOC, so the frontend UI
can render live charts just like TTS.
"""
...
def main():
    ...
    # --- Load Whisper from local snapshot ---
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_id = recipe.get("SourceRepo") or "openai/whisper-tiny.en"
    processor = WhisperProcessor.from_pretrained(local_model_dir, local_files_only=True)
    model = WhisperForConditionalGeneration.from_pretrained(local_model_dir, local_files_only=True).to(device)
    ...
    last = None
    for update in mc.compute_stt_metrics_stream(
    ...
    if is_indispensable:
        ...
        try:
            import torch
            probe_audio = torch.randn(1, 16000)
